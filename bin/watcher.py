...
RECORDED_RESPONSE_DIR = 'webapp/recorded_responses'
LOCAL_DIRS_TO_IGNORE = [BUILD_STATUS_DIR, RECORDED_RESPONSE_DIR]
...
class Syncer(threading.Thread):
    ...
    @property
    def source_dir(self):
        return os.path.basename(os.path.normpath(self.source)) + os.sep
    ...
    def watch_complete(self):
        self.path = None
        self.set_tab_title("Watching %s..." % self.source_dir)
    ...
    def sync_cb(self):
        with self.lock:
            ...
            delete_cmd = '' if self.options.preserve else '--delete'

            normalized_path = os.path.normpath(self.path)
            if os.path.isdir(normalized_path):
                normalized_path += os.sep

            excludes = {"*.pyc", "*.ldb", ".DS_Store", "build", "node_modules", "*.mo", "*.class", "*.zip", "*.gz",
                        "*.jar", RECORDED_RESPONSE_DIR, BUILD_STATUS_DIR, '.dependencies', 'build.log', 'webpack.log',
                        'app.log', 'api.log', 'node.log', '.eslintcache', '.docker_cid', '.eslintcachedir',
                        '*.iml', 'workspace.xml', 'api/experience_debug.log', 'bazel-*'}
            excludes = excludes.union(self.options.extra_excludes)
            excludes = excludes.difference(self.options.includes)
...
def file_changed(add_event, options, ignored_local_paths, subpath, mask):
    ...
    for path in ignored_local_paths:
        if subpath.startswith(path):
            return
    if options.notify:
        dirname = os.path.basename(os.path.normpath(options.source))
        cmd = ('reattach-to-user-namespace osascript -e \'display notification'
               '"Syncing changes for watcher" with title  "%s"\'' % dirname)
        call(shlex.split(cmd))
    add_event(subpath)
...
parser.add_argument("--notify", dest="notify", action="store_true", default=False,
                    help="Toggle this to get a mac notification everytime a file change has been detected")
parser.add_argument("--preserve", dest="preserve",
                    help="preserve files in destination not present on local machine "
                         "(instead of default behavior of using rsync --delete flag to delete them)",
                    action="store_true", default=False)
parser.add_argument(
    "--exclude", dest="extra_excludes", action="append", default=[])
parser.add_argument(
    "--include", dest="includes", action="append", default=[],
    help="Override specific items in the set of default excludes")
